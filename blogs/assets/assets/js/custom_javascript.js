...
/*function generateCaptcha() {
    ...
    //alert(captcha);
    if(document.getElementById('captcha_book_now').value)
    {
...
console.log(this.responseText);
  ...
  if(    new_string == 1 ){
                 document.getElementById('show_contact_error').innerHTML = "<p style='color:green;font-size:11px; padding-left:10px;'> Thank you for your enquiry. A member of our team will be in touch as soon as possible.</p>";
				 $('#contactSub').removeAttr("disabled");
          			$("#contactSub").text('SEND NOW');
				 form_reset = document.getElementById('contact_enquiry_form_kvch');
				 form_reset.reset();
				}
...
var k_v_c_h = document.getElementById('k_v_c_h').value;
...
if ( CaptchaInput == "" ||  course_form_full_name == "" || course_form_contact_no == "" || course_form_email == "" || course_form_query == "" || k_v_c_h == "" ){ 
 ...
 return false;event.preventDefault(); 
	  ...
	  $("#submit_wide").text('Please wait..');
  	$("#submit_wide").css('pointer-events','none');
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) { 
          console.log(this.responseText);
        ...
        if(new_string == 1 ){
                    document.getElementById('show_course_error').innerHTML = "<p style='color:green;font-size:11px; padding-left:10px;'> Thank you for your enquiry. A member of our team will be in touch as soon as possible.</p>";
                		$("#submit_wide").text('Submit');
         		        $("#submit_wide").css('pointer-events','auto');
				            form_reset = document.getElementById('submit_query_form_for_courses');
				            form_reset.reset();
              }
              else if(new_string  == 0 ){
        ...
        xmlhttp.open('POST', 'backend/sendcoursedetails.php?a='+course_form_full_name+'&b='+course_form_contact_no+'&c='+course_form_email+'&d='+course_form_query+'&e='+course_form_query_title+'&f='+course_form_course_name+'&g='+select_location_for_course_form+'&h='+k_v_c_h, true);
        xmlhttp.send();
    }
}
